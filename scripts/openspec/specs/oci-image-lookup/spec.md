...
Provide a read-only command-line tool, `find-oracle-daily`, that resolves the
OCID of a daily Ubuntu image published to Oracle Cloud Infrastructure for a
given suite, architecture, and family, selecting the highest available serial
unless a specific serial is requested.
